...
@description('fileshare role definition Id')
param fileshareRoleDefinitionId string
...
resource fileshareRoleAuthorization 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('storage-fileshare-rbac', storageAccount.id, resourceGroup().id, principalId, fileshareRoleDefinitionId)
  scope: storageAccount
  properties: {
      principalId: principalId
      roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions',fileshareRoleDefinitionId)
      principalType: empty(principalType) ? null : principalType
  }
}
